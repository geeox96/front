      ...
      <v-flex xl2 lg3 md3 sm6 xs12 v-for="liquido in juices" :key="liquido.id">
        <v-hover :open-delay="300">
          <template v-slot:default="{ hover }">
            <v-card class="mx-auto elevation-0" max-width="344" @click="abrirDetalheJuice(liquido)">
              <v-img :aspect-ratio="9/11" lazy-src :src="liquido.img" />
              <v-fade-transition>
                <v-overlay v-if="hover" opacity="0.70" absolute color="#000">
...
export default {
  ...
  mounted() {
    this.$store
      .dispatch("consultarLiquidosAtivos")
      .then(() => (this.juices = this.getTodosLiquidosAtivos));
